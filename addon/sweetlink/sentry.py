...
class Sentry:
    ...
    def _setupSentryInternal(versionString:str, distType:str, isDevMode:bool, canEnableProfiling:bool) -> None:
        ...
        # Setup and init
        sentry_sdk.init(
            dsn= "https://a96d03a14c702aa218170e9f0ab16b56@oe-reports.octoeverywhere.com/7",
            integrations= [
                sentryLogging,
                ThreadingIntegration(propagate_hub=True),
            ],
            # This is the recommended format
            release= f"hw-plugin@{versionString}",
            dist= distType,
    ...
    def _beforeSendFilter(event:Any, hint:Any):
        ...
        if Sentry.FilterExceptionsByPackage:
            ...
            try:
                ...
                for s in stack:
                    # Check for the name of our top level packages in the filename.
                    # This will match one of the main modules in our code, but exclude any 3rd party code.
                    # "sweetlink" covers both the sweetlink and sweetlink_linuxhost packages.
                    filenameLower = s.filename.lower()
                    if "sweetlink" in filenameLower:
