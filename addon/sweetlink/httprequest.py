    ...
    def MakeHttpCall(logger:logging.Logger, pathOrUrl:str, pathOrUrlType:int, method:str, headers:Optional[Dict[str, str]]=None, data:BufferOrNone=None, allowRedirects=False, apiTarget:Optional[int]=None) -> Optional[HttpResult]:
        ...
        if apiTarget is not None and (apiTarget == HaApiTarget.Core or apiTarget == HaApiTarget.Supervisor):
            ...
            if not HttpRequest.RemoteAccessEnabled:
                ...
                return HttpResult.Redirect("https://sweetplace.me/?errorType=remoteAccessDisabled", False)
        ...
        # For these to work properly on a remote browser, the Sweetplace service will detect and convert the URLs in to encoded relative
        # URLs for the portal. This ensures when the remote browser tries to access the HTTP endpoint, it will hit Sweetplace. The Sweetplace
        ...
        # on. However, for our setup its a little more complex. The issue is the Sweetplace plugin not knowing how the user's system is setup.
