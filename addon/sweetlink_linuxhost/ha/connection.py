...
from sweetlink.buffer import Buffer
from sweetlink.sentry import Sentry
from sweetlink.websocketimpl import Client
from sweetlink.interfaces import IWebSocketClient, WebSocketOpCode, IHomeAssistantWebSocket
...
class Connection(IHomeAssistantWebSocket):
    ...
    def __init__(self, logger:logging.Logger, eventHandler:EventHandler) -> None:
        ...
        self.HaVersionString:Optional[str] = None
    ...
    def SendAndReceiveMsg(self, msg:Dict[str, Any], timeout:float=30.0) -> Optional[Dict[str, Any]]:
        return self.SendMsg(msg, waitForResponse=True, timeout=timeout)
    ...
    def SendMsg(self, msg:Dict[str, Any], waitForResponse:bool=False, ignoreConnectionState:bool=False, timeout:float=30.0) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            ws.Send(Buffer(jsonStr.encode()), isData=False)
            ...
            if pendingContext.Event.wait(timeout) is False:
